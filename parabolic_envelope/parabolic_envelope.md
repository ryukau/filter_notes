# 放物線エンベロープ
加速度が一定のとき、物体は放物線を描いて移動します。この放物線を使ってエンベロープを作ります。

次の図は加速度が一定のエンジンを積んだロケットをある点から別の点へ移動させて戻ってくるモデルを表しています。

![](img/desired_curve.svg)

## 加速度を任意に決める方法
アタックの区間での加速度 $a_A$ と $b_A$ 、加速から減速に切り替える時点 $t_A$ をユーザが指定するとします。

曲線の最大値となるピーク $h_p$ とその時点 $t_p$ を求めます。
...
$t_R = t_p + \beta l_R$ と定義します。 $\beta$ はユーザが操作できるパラメータで範囲は $0 < \beta < 1$ です。
...
$$
d = \frac{2}{l_R}, \quad t_R = t_p + \beta l_R
$$
...
$$
\begin{aligned}
a_R \beta l_R &= \frac{2}{l_R} \\
a_R &= \frac{2}{\beta l_R^2} \\
\end{aligned}
$$
...
$$
\begin{aligned}
b_R (1 - \beta) l_R &= \frac{2}{l_R} \\
b_R &= \frac{2}{(1 - \beta) l_R^2} \\
\end{aligned}
$$
...
出力です。この実装も終端の処理が適当なので小さなギャップがあります。図より $\beta$ の方向がアタックとリリースで逆になっていることが分かります。
