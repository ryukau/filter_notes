...
struct LinearInterp {
  ...
  LinearInterp(float value = 0) : v0(value), v1(value) {}
...
struct DSP {
  LinearInterp gain{1.0f};

  void process(const size_t frame, float *out)
  {
    for (size_t i = 0; i < frame; ++i) out[i] = gain.process(frame, i);
  }
};
...
int main()
{
  ...
  return writeWave("snd/linterp.wav", wav, sampleRate);
}
