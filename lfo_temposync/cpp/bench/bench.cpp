...
template<typename Synchronizer> void bench(std::string name)
{
  ...
  for (size_t loop = 0; loop < nLoop; ++loop) {
    ...
    while (frame < wavSize) {
      ...
      syncer.prepare(sampleRate, tempo, sync, float(elapsedBeats), false);
      ...
      elapsedBeats += tempo * bufferSize / (60.0 * sampleRate);
      if (elapsedBeats > 4.7f) elapsedBeats = 0.0f;
    }
  }
